...
sheet_url <- "https://docs.google.com/spreadsheets/d/1zDLZ5miZ0zkxt3uwPoNs-wWiTkUkr5bfBY41Hj-sqck/edit?gid=0#gid=0"
...
spp_biodiv_df <- read_sheet(sheet_url,
      ...
      janitor::clean_names() %>% 
      dplyr::mutate(across(hassler_expedition:species_occurrence, ~ as.numeric(.x))) %>% 
  filter(!is.na(benthic_pelagic),
         benthic_pelagic != "N/A")
          # Filter out rows without benthic/pelagic info


length(unique(spp_biodiv_df$scientific_name)) #624
...
fam_biodiv_df <- read_sheet(sheet_url,
      ...
      janitor::clean_names() %>% 
      dplyr::mutate(across(hassler_expedition:family_occurrence, ~ as.numeric(.x))) %>% 
  filter(!is.na(benthic_pelagic),
         benthic_pelagic != "N/A")
...
run_biodiv_analysis <- function(df_subset) {
      ...
      community_matrix_pres <- df_subset %>%
            pivot_longer(cols = hassler_expedition:rv_atlantis_at21_02, # Note these must include all columns on data
                         names_to = "site", 
                         values_to = "value") %>%
      ...
      iNEXT(inc_freq, q = 0, datatype = "incidence_freq", nboot = 250, endpoint = 700) # q and qdp is what plots
}
...
bio_spp <-
  ...
  geom_ribbon(data = Hills_q0_panel_df %>% filter(Group != "Benthic"),
              aes(x = t, ymin = qD.LCL,ymax = qD.UCL, color = Method),
              alpha = 0.2, show.legend = FALSE) +        
  geom_ribbon(data = Hills_q0_panel_df %>% filter(Group != "Pelagic"),
              aes(x = t, ymin = qD.LCL,ymax = qD.UCL, color = Method),
              alpha = 0.2, show.legend = FALSE) +        
  geom_line(aes(x = t,
                y = qD,
...
ggsave(bio_spp,
       filename = "../results/benthic_pelagic_diversity_a.png",
       width = 9,
       height = 7)
...
# Save dataset
write_csv(
          "../results/spp_diversity_indices_bp_new_spp.csv")
...
inc_freq <- as.incfreq(as.matrix(t(community_matrix_pres)))
Hills_q_0_inc_CCZ <- iNEXT(inc_freq, q = 0, datatype = "incidence_freq", nboot = 250, endpoint = 250) # q and qdp is what plots
...
bio_fam <-
  ...
  geom_point(data = Hills_q0_inc_df %>% filter(Method == 'Observed'), aes(x = t, y = qD, color = Method), cex = 4) +
  xlab("") +
...
write_csv(diversity_indices %>% mutate(category = c("Species Richness", "Shannon div.", "Simpson div"),
                                             ,level = "family",n = 11) %>% 
                select(category,everything()),
          "../results/fam_diversity_indices_new_spp.csv")
...
ggsave(
      "../results/diversity_plots_bp_line_new_spp.png",
      pp,
      width = 7,
      height = 5
)
...
# Species curve by depth boxes

```{r}
# spp_biodiv_df <- my_path("D", name = "spp_biodiversity_data.csv", read = T)


sheet_url <- "https://docs.google.com/spreadsheets/d/1zDLZ5miZ0zkxt3uwPoNs-wWiTkUkr5bfBY41Hj-sqck/edit?usp=sharing"
...
spp_biodiv_df %>% 
      filter(!is.na(depth_boxes)) %>%
      group_by(benthic_pelagic,depth_boxes) %>% 
      summarise(n = n()) %>% 
  group_by(benthic_pelagic) %>%
  tally()
      #visualize boxes in each category
      ggplot(aes(x = depth_boxes, y = n, fill = benthic_pelagic)) +
      ...
      coord_flip() +
      facet_wrap(~ benthic_pelagic, ncol = 2, scale = "free_y")

ggsave("../results/depth_boxes_distribution.png",
       last_plot(),
       width = 8,
       height = 14)
...
spp_biodiv_df %>% 
  mutate(
    depth = as.numeric(depth_boxes)
  ) %>%
  filter(!is.na(depth)) %>% 
  ggplot(aes(x = depth, fill = benthic_pelagic)) +
